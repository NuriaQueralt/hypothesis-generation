...
# query topology
query_topology = """
(source)--(ds:DISO)--(:GENE)-[:`RO:HOM0000020`]-(g1:GENE)--(pw:PHYS)--(:GENE)--(target)
"""
...
with driver.session() as session:
    ...
    for node1 in seed:
        for node2 in seed:
            ...
            RETURN count(distinct path) AS paths, count(distinct pw) as pathways, count(distinct ds) as diseases,  count( distinct other_ortho ) AS other_ortho
            ORDER BY other_ortho DESC
            """
    ...
    with open('./out/{}.tsv'.format(args.output), 'w') as f:
        f.write('source\ttarget\tpaths\tpathways\tdiseases\tother_orthologs\n')
        for pairwise in output:
            f.write('{}\t{}\t{}\t{}\t{}\t{}\n'.format(pairwise['source'], pairwise['target'], pairwise['paths'], pairwise['pathways'], pairwise['diseases'], pairwise['other_ortho']))
