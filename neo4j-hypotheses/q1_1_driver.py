...
for index, row in df.iterrows():
    ...
    if ( row.max_pathway_degree == 0 and row.max_disease_degree == 0 ):
        output_filename = "{}_in".format(query) + str(index) + "_pwdl50_phdl20_paths"
        cmd = "python3 {}_cypher_to_hypotheses.py -o ".format(query) + output_filename
    elif ( row.max_pathway_degree == 0):
        output_filename = "{}_in".format(query) + str(index) + "_pwdl50_phdl" + str(row.max_disease_degree) + "_paths"
        ...
        output_filename = "{}_in".format(query) + str(index) + "_pwdl" + str(row.max_pathway_degree) + "_phdl20_paths"
        cmd = "python3 {}_cypher_to_hypotheses.py -pwdl ".format(query) + str(row.max_pathway_degree) + " -o " + output_filename
    else:
        output_filename = "{}_in".format(query) + str(index) + "_pwdl" + str(row.max_pathway_degree) + "_phdl" + str(row.max_disease_degree) + "_paths"
